...
tab2 <- as.data.frame(tab %>% pivot_longer(cols = c(COVID_ICU, COVID_INPAT, COVID_NEG, COVID_OUTPAT)))

p1 <- tab2 %>% ggplot(aes(x=Var1, y=value, fill = name)) + geom_bar(position = "fill", stat="identity") + coord_flip()
p1 <- p1 + theme(axis.text.y = element_text(size = 6))
p1
```
